...
var SankeyDriver = function (){
  ...
  this.draw = function (inputdata) {
    ...
    function funcMousemove() {
      // Adjust left and top offset so that the tooltip doesn't go off-screen
      var canvasWidth = $('#canvas').width();
      var canvasHeight = $('#canvas').height();
      var tooltipWidth = tooltipContainer[0][0].clientWidth;
      var tooltipHeight = tooltipContainer[0][0].clientHeight;
      var offsetTop = d3.event.clientY + tooltipHeight > canvasHeight ? Math.max(canvasHeight - tooltipHeight, 0) : d3.event.clientY;
      var offsetLeft = d3.event.clientX + tooltipWidth > canvasWidth ? Math.max(canvasWidth - tooltipWidth, 0) : d3.event.clientX;
      tooltipContainer
        .style('top', offsetTop /*d3.event.clientY*/ + 'px')
        .style('left', offsetLeft /*d3.event.clientX*/ + 'px');
    }
